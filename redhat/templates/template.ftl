...
var updateCredentials = function(username, password) {
    if(window.webkit) {
        var msg = {
            subType: 'sso-credentials',
            username,
            userPasswordHash: password && forge_sha256(password)
        };
...
if(updateForm) {
    updateForm.addEventListener("submit", function(e){
        var passwordNew = e.target['password-new'].value;
        var passwordConfirm = e.target['password-confirm'].value;
